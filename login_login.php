<?php
	...
	$qryspts = $db->prepare("SELECT * FROM user WHERE email='$email' AND password='$password' AND utype='Spare_Parts' AND stat='Active'");
	$qryspts->execute();
	...
	if($counta > 0) {
		...
		$_SESSION['SESS_SPAREPARTS_ID'] = $rowspts['Log_Id'];
		session_write_close();
		header("location: spare_parts/index.php");
		exit();
	}
	else 
	{
